...
class EventCalendar < Page
  ...
  tag "calendar" do |tag|
    ...
    # p = Hash[:group => path[2], :calendar => path[3], :period => path[4]]
    # p.default = nil
    # tag.locals.debug = "Group:" << p[:group].to_s << "<br />Calendar:" << p[:calendar].to_s << "<br />Period:" << p[:period].to_s
    # 
    tag.locals.calendar_group = tag.attr['calendar_group'] || @request.path_parameters[:calendar_group] || "master"
    tag.locals.calendars = tag.attr['calendar'] || @request.path_parameters[:calendar] || "all"
    tag.locals.period = tag.attr['period'] || @request.path_parameters[:period] ||"month"
